...
hourarray=hourin1:(1/60):hourin2;
...
SEC=ones(length(H),1)*(3600)*(hourin2+hourin1)/2;
LST=(SEC/3600 + LON/15);
...
fileref=['profile',num2str(counter+9000),'.data'];

%write file to output
% for MAGIC
if codeflag==1
profile1=[height' dox' dnit2' dox2' dens' temp' dhyd' dne' windU' windV'];
dlmwrite(fileref,profile1,'delimiter','\t','precision','%.6E','-append');
end
...
figure(406)
subplot(2,1,1)
imagesc(hourarray,height,Ufinal)
axis xy
colorbar
xlabel('Time (UT)')
ylabel('Altitude')
title(['U (m/s) on ',datestr(d)])

subplot(2,1,2)
imagesc(hourarray,height,Vfinal)
axis xy
colorbar
xlabel('Time (UT)')
ylabel('Altitude')
title(['V (m/s) on ',datestr(d)])
