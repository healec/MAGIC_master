...
c
      ...
      if (use_fwave.EQV..FALSE.) then 
         print*,'Sorry, this is an f-wave solver.'
         stop
      end if
      limit = .false.
      do 5 mw=1,mwaves
...
c      if (limit) call limiter(maxm,meqn,mwaves,mbc,mx,fwave,s,mthlim)
      if (limit) then
         print*,'This version requires fwaves to be limited in rpn3.'
         stop
      endif 
c
...
c
      call rpt3(ixyz,2,1,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &          aux1,aux2,aux3,amdq,bmamdq,bpamdq)
c
c     # split the right-going flux difference into down-going and up-going
c     # flux differences (in the y-direction).
c
      call rpt3(ixyz,2,2,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &          aux1,aux2,aux3,apdq,bmapdq,bpapdq)
c
c     # split the left-going flux difference into down-going and up-going
c     # flux differences (in the z-direction).
c
      call rpt3(ixyz,3,1,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &          aux1,aux2,aux3,amdq,cmamdq,cpamdq)
c
c     # split the right-going flux difference into down-going and up-going
c     # flux differences (in the y-direction).
c
      call rpt3(ixyz,3,2,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
...
c            # in the y-like direction with imp=1
              call rpt3(ixyz,2,1,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &            aux1,aux2,aux3,cqxx,bmcqxxm,bpcqxxm)

c            # in the y-like direction with imp=2
              call rpt3(ixyz,2,2,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &            aux1,aux2,aux3,cqxx,bmcqxxp,bpcqxxp)

c            # in the z-like direction with imp=1
              call rpt3(ixyz,3,1,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &            aux1,aux2,aux3,cqxx,cmcqxxm,cpcqxxm)

c            # in the z-like direction with imp=2
              call rpt3(ixyz,3,2,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &            aux1,aux2,aux3,cqxx,cmcqxxp,cpcqxxp)
          else
...
c            # in the y-like direction 
              call rpt3(ixyz,2,0,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &              aux1,aux2,aux3,cqxx,bmcqxxm,bpcqxxm)

c            # in the z-like direction 
              call rpt3(ixyz,3,0,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &              aux1,aux2,aux3,cqxx,cmcqxxm,cpcqxxm)
...
c
      if( m4.gt.0 )then
          call rptt3(ixyz,2,2,2,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &              aux1,aux2,aux3,cpapdq2,bmcpapdq,bpcpapdq)
          call rptt3(ixyz,2,1,2,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &              aux1,aux2,aux3,cpamdq2,bmcpamdq,bpcpamdq)
          call rptt3(ixyz,2,2,1,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &              aux1,aux2,aux3,cmapdq2,bmcmapdq,bpcmapdq)
          call rptt3(ixyz,2,1,1,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
...
c
      if( m4.gt.0 )then
          call rptt3(ixyz,3,2,2,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &              aux1,aux2,aux3,bpapdq,bmcpapdq,bpcpapdq)
          call rptt3(ixyz,3,1,2,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &              aux1,aux2,aux3,bpamdq,bmcpamdq,bpcpamdq)
          call rptt3(ixyz,3,2,1,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
     &              aux1,aux2,aux3,bmapdq,bmcmapdq,bpcmapdq)
          call rptt3(ixyz,3,1,1,maxm,meqn,mwaves,maux,mbc,mx,q1d,q1d,
