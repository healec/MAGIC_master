...
c     ==================================================================
     ...
     &           use_fwave,info,bc3,rpn3,rpt3,rptt3,src3,b4step3)
c     ==================================================================
c
c     --------------------------------------------------------
...
c       conditions.
c
c    q(1-mbc:mx+mbc, 1-mbc:my+mbc, 1-mbc:mz+mbc,meqn)
...
c
c    aux(1-mbc:mx+mbc, 1-mbc:my+mbc, 1-mbc:my+mbc,maux)
...
c
c        # MPI: pass time-varying aux array data between processors, 
c        #      so that ghost cell values in aux arrays are set 
c        #      correctly.
c        # MPI: UNCOMMENT IF aux ARRAYS CHANGE WITH TIME.
c$$$         call bc3_aux(meqn,mbc,mx,my,mz,xlower,
c$$$     &         ylower,zlower,dx,dy,dz,q,maux,aux,told,dt,mthbc)
c
c
...
c            # source terms over a half time step:
     ...
     &                 q,maux,aux,told,dt2,mthbc)
c            # Boundary conditions must be reset here
             call bc3(meqn,mbc,mx,my,mz,xlower,ylower,zlower,
     &                 dx,dy,dz,q,maux,aux,told,dt2,mthbc)
             endif
c
c        # copy q into qwork1.  q is updated in step2 and qwork1 is
c        # preserved to provide data for Riemann problems.
c
         call copyq3(meqn,mbc,mx,my,mz,q,work(i0qwrk1))
c
...
c
     ...
     &                 work(i0next),mwork1,rpn3,rpt3,rptt3,use_fwave)
c
         else
...
c        # Get the maximum cfl number on all processors; use this to determine
c        # next time step.  This way, all processors will be synchronized in
c        # time.
         call MPI_ALLREDUCE(cfl,cfl_global,1,MPI_DOUBLE_PRECISION,
     &        MPI_MAX,MPI_COMM_WORLD,ierr)
c$$$         call MPI_REDUCE(cfl,cfl_global,1,MPI_DOUBLE_PRECISION,
c$$$     &        MPI_MAX,np-1,MPI_COMM_WORLD,ierr)
c$$$         call MPI_BCAST(cfl_global,1,MPI_DOUBLE_PRECISION,
c$$$     &        np-1,MPI_COMM_WORLD,ierr)
         cfl = cfl_global
c
         if ((method(4).eq.1) .and. (id.eq.0)) then
c        # verbose mode
...
c             # solution is stored in both cases method(5) = 0 or 1.
              t = told
              call copyq3(meqn,mbc,mx,my,mz,work(i0qwrk2),q)
c
              if (method(4).eq.1) then
c                # verbose mode
                 if (id.eq.0) write(6,602)
