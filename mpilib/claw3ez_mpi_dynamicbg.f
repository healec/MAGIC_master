...
c
      do i = 0,np-1
         if (id.eq.i) then
            call setprob
            write(*,*) 'Processor ', i, ' done with setprob'
         end if
...
c
      call qinit(meqn,mbc,mx,my,mz,xlower,ylower,
...
c        EVERY TIMESTEP AS OUTPUT - CHANGE BACKGROUND
         if (mod(n,1) .eq. 0.0d0) then
         ...
         do k = 1-mbc,mz+mbc
            do j = 1-mbc,my+mbc
               do i = 1-mbc,mx+mbc
                  do m = 1,meqn
                     q(m,i,j,k) = q(m,i,j,k)-aux(m,i,j,k)
                  enddo
               enddo
            enddo
         enddo
     ...
     &               zlower,dx,dy,dz,maux,iframe,aux)
         ...
         do k = 1-mbc,mz+mbc
            do j = 1-mbc,my+mbc
               do i = 1-mbc,mx+mbc
                  do m = 1,meqn
                     q(m,i,j,k) = aux(m,i,j,k)+q(m,i,j,k)
                  enddo
               enddo
            enddo
         enddo
